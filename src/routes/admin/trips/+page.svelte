<script lang="ts">
	...
	// Admin Trip Delete modal state
	let deletingTrip = $state<any>(null);
	let deleteConfirmInput = $state('');

	function openDeleteTripModal(trip: any) {
		deletingTrip = trip;
		deleteConfirmInput = '';
	}
					...
					<tr class="master-row {pastDue ? 'row-past-due' : ''}" onclick={() => toggleTrip(trip.id)}>
						...
						<td>
							<div class="row-actions" onclick={(e) => e.stopPropagation()} role="presentation">
								...
								<button
									type="button"
									class="btn btn-xs btn-danger btn-delete-trip-icon"
									title="Delete Trip Instance"
									onclick={() => openDeleteTripModal(trip)}
								>
...
{#if deletingTrip}
	{@const deleteTemplate = (Array.isArray(deletingTrip.listing_templates) ? deletingTrip.listing_templates[0] : deletingTrip.listing_templates) as any}
	{@const deleteBookings = deletingTrip.bookings || []}

	<button class="drawer-backdrop" onclick={closeDeleteTripModal} aria-label="Close modal"></button>
	<div class="admin-cancel-modal glass glow-danger" role="dialog" aria-modal="true">
		<div class="modal-header">
			<div>
				<span class="drawer-subtitle">Operations Overrides</span>
				<h2 style="color: var(--danger);">Delete Trip Instance</h2>
				<p class="modal-trip-meta">{deleteTemplate?.trip_type || 'Charter'} — {formatDate(deletingTrip.date)} ({deleteTemplate?.location || 'Unknown'})</p>
			</div>
			<button class="close-btn" onclick={closeDeleteTripModal} aria-label="Close Modal">
				<svg class="close-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
					<path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
				</svg>
			</button>
		...
		>
			...
			<div class="modal-body space-y-4">
				<div class="affected-summary-card glass">
					<p class="affected-title"><strong>Trip Details:</strong></p>
					<ul class="customer-list">
						<li><span><strong>Date:</strong> {formatDate(deletingTrip.date)}</span></li>
						<li><span><strong>Trip Type:</strong> {deleteTemplate?.trip_type || 'Charter'}</span></li>
						<li><span><strong>Status:</strong> {deletingTrip.status}</span></li>
						<li><span><strong>Bookings:</strong> {deleteBookings.length} group(s) booked</span></li>
					</ul>
				</div>

				<div class="alert alert-danger glass" style="padding: 1rem; border-radius: 8px; border: 1px solid rgba(239,68,68,0.3); background: rgba(239,68,68,0.1); color: #fca5a5;">
					<p style="margin: 0; font-size: 0.9rem;"><strong>Warning:</strong> Permanently deleting this trip instance will remove all associated bookings and records from the database. This action cannot be undone.</p>
				</div>

				<div class="form-group" style="display: flex; flex-direction: column; gap: 8px;">
					<label for="confirm-delete-input" class="form-label" style="font-size: 0.9rem; font-weight: 600; color: var(--text-secondary);">
					...
					<input
						...
						class="reason-textarea"
						style="padding: 10px 14px; height: auto; resize: none;"
						autocomplete="off"
					/>
				</div>
			</div>
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary" onclick={closeDeleteTripModal}>
					Cancel
				</button>
				<button
					type="submit"
					...
					style="background: var(--danger); border-color: var(--danger); opacity: {deleteConfirmInput.trim() === 'DELETE' ? 1 : 0.4}; cursor: {deleteConfirmInput.trim() === 'DELETE' ? 'pointer' : 'not-allowed'};"
				>
